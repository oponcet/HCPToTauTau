...
setup_hcp() {
    ...
    #   - "HCP_", for features provided by the analysis repository itself.
    # Check the setup.sh in columnflow for documentation of the "CF_" variables. The purpose of all
    # "HCP_" variables is documented below.
    #
    ...
    if [ "${HCP_SETUP}" = "1" ]; then
        >&2 echo "the HCPToTauTau analysis was already succesfully setup"
    ...
    #
    # global variables
    # (HCP = HCPToTauTau, CF = columnflow)
    #

    # start exporting variables
    export HCP_BASE="${this_dir}"
    export CF_BASE="${this_dir}/modules/columnflow"
    export CF_REPO_BASE="${HCP_BASE}"
    export CF_REPO_BASE_ALIAS="HCP_BASE"
    ...
    if [ "${CF_REMOTE_JOB}" != "1" ]; then
        cf_setup_interactive_body() {
            ...
            query CF_DATA "Local data directory" "\$HCP_BASE/data" "./data"
            query CF_STORE_NAME "Relative path used in store paths (see next queries)" "hcp_store"
            query CF_STORE_LOCAL "Default local output store" "\$CF_DATA/\$CF_STORE_NAME"
            query CF_WLCG_CACHE_ROOT "Local directory for caching remote files" "\$CF_DATA/hcp_cache"
            ...
            query HCP_BUNDLE_CMSSW "Install and bundle CMSSW sandboxes for job submission?" "True"
        }
        cf_setup_interactive "${CF_SETUP_NAME}" "${HCP_BASE}/.setups/${CF_SETUP_NAME}.sh" || return "$?"
    fi
    ...
    export PATH="${HCP_BASE}/bin:${PATH}"
    export PYTHONPATH="${HCP_BASE}:${HCP_BASE}/modules/cmsdb:${PYTHONPATH}"

    # initialze submodules
    if [ -e "${HCP_BASE}/.git" ]; then
        local m
        for m in $( ls -1q "${HCP_BASE}/modules" ); do
            cf_init_submodule "${HCP_BASE}" "modules/${m}"
        done
    fi
    ...
    export LAW_HOME="${HCP_BASE}/.law"
    export LAW_CONFIG_FILE="${HCP_BASE}/law.cfg"
...
main() {
    ...
    if setup_hcp "$@"; then
        cf_color green "HCPToTauTau analysis successfully setup"
        return "0"
    else
        local code="$?"
